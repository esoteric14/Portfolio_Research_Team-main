...
def retry_yfinance_call(callable_fn, *args, max_retries=5, **kwargs):
    """Retry a yfinance call with exponential backoff on 429 errors."""
    for attempt in range(max_retries):
        try:
            ...
            if e.response is not None and e.response.status_code == 429:
                wait = 2 ** attempt
                logger.warning(f"Rate limited by Yahoo Finance. Waiting {wait} seconds before retrying...")
                time.sleep(wait)
            else:
                raise
        except Exception as e:
            # yfinance sometimes wraps HTTPError in generic Exception
            if hasattr(e, 'response') and getattr(e.response, 'status_code', None) == 429:
                ...
                raise
    raise Exception("Max retries exceeded for yfinance call")
...
def get_historical_stock_prices_sync(ticker, period, interval):
    ...
    company = retry_yfinance_call(get_company)
    if company.isin is None:
        logger.error(f"Company ticker {ticker} not found.")
        return json.dumps({"error": f"Company ticker {ticker} not found."})
    hist_data = retry_yfinance_call(company.history, period=period, interval=interval)
...
def get_stock_info_sync(ticker):
    ...
    if company.isin is None:
        logger.error(f"Company ticker {ticker} not found.")
        return json.dumps({"error": f"Company ticker {ticker} not found."})
    info = retry_yfinance_call(lambda: company.info)
...
def get_yahoo_finance_news_sync(ticker):
    ...
    if company.isin is None:
        ...
        return json.dumps({"error": f"Company ticker {ticker} not found."})
    try:
        news = retry_yfinance_call(lambda: company.news)
    except Exception as e:
...
def get_stock_actions_sync(ticker):
    ...
    actions_df = retry_yfinance_call(lambda: company.actions)
...
def get_financial_statement_sync(ticker, financial_type):
    ...
    if financial_type == FinancialType.income_stmt:
        financial_statement = retry_yfinance_call(lambda: company.income_stmt)
    elif financial_type == FinancialType.quarterly_income_stmt:
        financial_statement = retry_yfinance_call(lambda: company.quarterly_income_stmt)
    elif financial_type == FinancialType.balance_sheet:
        financial_statement = retry_yfinance_call(lambda: company.balance_sheet)
    elif financial_type == FinancialType.quarterly_balance_sheet:
        financial_statement = retry_yfinance_call(lambda: company.quarterly_balance_sheet)
    elif financial_type == FinancialType.cashflow:
        financial_statement = retry_yfinance_call(lambda: company.cashflow)
    elif financial_type == FinancialType.quarterly_cashflow:
        financial_statement = retry_yfinance_call(lambda: company.quarterly_cashflow)
    else:
...
def get_holder_info_sync(ticker, holder_type):
    ...
    if holder_type == HolderType.major_holders:
        df = retry_yfinance_call(lambda: company.major_holders.reset_index(names="metric"))
    elif holder_type == HolderType.institutional_holders:
        df = retry_yfinance_call(lambda: company.institutional_holders)
    elif holder_type == HolderType.mutualfund_holders:
        df = retry_yfinance_call(lambda: company.mutualfund_holders)
    elif holder_type == HolderType.insider_transactions:
        df = retry_yfinance_call(lambda: company.insider_transactions)
    elif holder_type == HolderType.insider_purchases:
        df = retry_yfinance_call(lambda: company.insider_purchases)
    elif holder_type == HolderType.insider_roster_holders:
        df = retry_yfinance_call(lambda: company.insider_roster_holders)
    else:
...
def get_option_expiration_dates_sync(ticker):
    ...
    if company.isin is None:
        logger.error(f"Company ticker {ticker} not found.")
        return json.dumps({"error": f"Company ticker {ticker} not found."})
    dates = retry_yfinance_call(lambda: list(company.options))
...
def get_option_chain_sync(ticker, expiration_date, option_type):
    ...
    if company.isin is None:
        logger.error(f"Company ticker {ticker} not found.")
        return json.dumps({"error": f"Company ticker {ticker} not found."})
    if expiration_date not in retry_yfinance_call(lambda: company.options):
    ...
    option_chain = retry_yfinance_call(lambda: company.option_chain(expiration_date))
...
def get_recommendations_sync(ticker, recommendation_type, months_back=12):
    ...
    try:
        if recommendation_type == RecommendationType.recommendations:
            df = retry_yfinance_call(lambda: company.recommendations)
        elif recommendation_type == RecommendationType.upgrades_downgrades:
            upgrades_downgrades = retry_yfinance_call(lambda: company.upgrades_downgrades.reset_index())
            ...
            logger.error(
                f"Invalid recommendation type {recommendation_type} for {ticker}."
            )
            return json.dumps(
